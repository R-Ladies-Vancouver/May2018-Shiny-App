...
server <- function(input, output) {
  ...
  output$GapminderPlot <- renderPlot({
    ...
    dat %>% filter(year >= min(input$Year) & year <= max(input$Year) & country %in% input$Countries) %>%
  ...
  output$GapminderTable <- renderTable({
    ...
    data <- dat %>% filter(year >= min(input$Year) & year <= max(input$Year) & country %in% input$Countries) ## filter data for seleted year range
